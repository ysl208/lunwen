In the previous chapters we completed preliminary steps to validate that end-users can easily learn the main concepts of the automated planning language and that the proposed robot programming paradigm is intuitive (\chapt{chap:Pre-Experiments}).
We then implemented a system that uses keyframe-based PbD and validated that it can be used easily to teach a variety of low-level actions (\chapt{chap:OrganisingTasks}).
Our main focus now lies in the implementation of the framework proposed in \chapt{chap:Contribution}.
In this chapter we present iRoPro, an end-to-end system for teaching robots primitive actions to be reused with a task planner (\fig{fig:overview}).
...
In the following we first present our approach on learning low- and high-level action representations from demonstration and generalising and reusing them with a task planner (\sect{sec:approach}).
...
In \sect{sec:syseval} we demonstrate our system's capability to generalise primitive actions on six benchmark tasks that are programmed and executed on the Baxter robot.
In \sect{sec:quanteval} we empirically investigate the usability of our system and validate its intuitiveness through a study with 21 users of different educational backgrounds and programming levels.
...
\label{sec:highlevel}
We represent high-level actions as proposed in task planning (\sect{subsec:Classical planning problem}), where an action is represented as a tuple $a = (\text{param}(a), \text{pre}(a),$ $\text{eff}(a))$, whose elements are:
\begin{itemize}
...
The source code for iRoPro is developed in ROS (\cite{quigley2009ros}) and available online\footnote{todo}. %ysl208/rapid\_pbd}.
...
where the bounding box is used to infer the object type (\sect{sec:inference}).
