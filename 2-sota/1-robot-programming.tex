...
We identified three key factors: robot behaviour, end-user, and data.
\begin{itemize}
	\item Control over the robot behaviour: if the robot behaviour code is directly entered and how it is generated (text-based or visual programming) and if the code is directly encodes the robot's executed behaviour (manual or automatic programming)
	\item End-user involvement: such as writing code manually, providing (labelled) data or continuous feedback during the learning process, defining reward functions
	\item Data: if the robot learns from biased data and if it has to acquire the data by exploration
\end{itemize}
...
We first distinguish between the user's control over the robot behaviour, i.e. manual vs. automatic programming systems.
