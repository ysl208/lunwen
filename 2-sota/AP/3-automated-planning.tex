...
Applying an action $a$ to state $s$ produces a new state $s'= \gamma(s,a)$ that is represented as an arc from $s$ to $s'$, referred to as a  \textit{state transition} (\fig{fig:state-transition-system}).
%with a cost $c(a)$
...
The planner can generate one of the following two action sequences (\fig{fig:planning problem}c):
...
Even though both plans consist of different actions, they both lead to the same goal states (\fig{fig:state-transition-system}).
