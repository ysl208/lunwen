...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.7\linewidth]{figures/toh-solutions}
...
We conclude this section by introducing the most significant planners for classical planning (Table \ref{tab:planningapproaches}), that have been implemented to date and open-source planning toolkits.
...
A second category are plan-space planners (\cite{penberthy1992ucpop}), where each node is a set of partially instantiated-operators with constraints.
...
Another category are planning-graph planners, first proposed in the Graphplan (\cite{blum:97}).
It uses a backward search by encoding the search space in a structure called the \textit{planning graph} and replaces the goal with preconditions.
...
The main planning toolkits are the Lightweight Automated Planning ToolKit (\cite{lapkt}), based on Fast Downward and Fast Forward, and PDDL4J, an open-source Planning Domain Description Library for Java cross-platform developers (\cite{pellier2018pddl4j}).
\begin{table}[h]
	\centering
	\caption{Main Planning Approaches}
	\label{tab:planningapproaches}
	\begin{center}
		\begin{tabular}{rll} \hline
			Approach & Generated Plan & Planning Systems\\ \hline
...
Finally we evaluate the implemented framework with another user study (\chapt{chap:Evaluation}).
